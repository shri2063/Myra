...
# skeleton pos
with open('data/00006_00/00006_00_keypoints.json', 'r') as f:
    ...
    print('s pos shape', s_pos.shape)

# cloth pos
with open('data/00006_00/cloth_landmark_json.json', 'r') as f:
    c_pos = json.load(f)
...
# parse
parse = Image.open('data/00006_00/parse.png')
parse = transforms.Resize(768)(parse)
parse = torch.from_numpy(np.array(parse)[None]).long()
def get_image():
    image = cv2.imread('data/00006_00/image.jpg', cv2.IMREAD_COLOR)
...
def get_cloth():
    cloth = cv2.imread('data/00006_00/cloth.jpg', cv2.IMREAD_COLOR)
    return  np.array(transforms.Resize(768)(Image.fromarray(cloth)))

def get_ag_mask():
    ag_mask = 255 - cv2.imread('data/00006_00/ag_mask.png', cv2.IMREAD_GRAYSCALE)
    return np.array(transforms.Resize(768)(Image.fromarray(ag_mask)))
def get_skin_mask():
    skin_mask = cv2.imread('data/00006_00/skin_mask.png', cv2.IMREAD_GRAYSCALE)
    return np.array(transforms.Resize(768)(Image.fromarray(skin_mask)))

def get_parse_13():
    im_parse_pil_big = Image.open('data/00006_00/paired_full_parse.png') # 768x1024
...
def get_e_pos():
    with open('data/00006_00/paired-ck-point.json', 'r') as f:
        e_pos = json.load(f)
