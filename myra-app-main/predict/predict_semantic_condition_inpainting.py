...
def create_gaussian_diffusion(
    *,
    ...
    betas = get_named_beta_schedule(noise_schedule, steps, use_scale=True)
...
def parse_args():
    ...
    # SCI Arguments
    args_sci = Default_Conf()
    args_sci.update({'attention_resolutions': '32,16,8', 'num_classes': 13, 'class_cond': True, 'diffusion_steps': 1000, 'learn_sigma': True, 'noise_schedule': 'linear', 'num_channels': 128, 'num_head_channels': 32, 'num_heads': 4, 'num_res_blocks': 2, 'resblock_updown': True, 'use_fp16': False, 'use_scale_shift_norm': True, 'classifier_scale': 0.0, 'lr_kernel_n_std': 2, 'num_samples': 100, 'show_progress': True, 'timestep_respacing': '50', 'use_kl': False, 'predict_xstart': False, 'rescale_timesteps': False, 'rescale_learned_sigmas': False, 'num_heads_upsample': -1, 'channel_mult': '', 'dropout': 0.0, 'use_checkpoint': True, 'use_new_attention_order': False, 'no_instance': True, 'clip_denoised': True, 'use_ddim': False, 'latex_name': 'VTON', 'method_name': 'VTON', 'image_size': 1024, 'model_path': 'checkpoints_pretrained/sci/ckpt_1024/ema_0.9999_300000.pt', 'name': 'demo', 'inpa_inj_sched_prev': True, 'n_jobs': 1, 'up': True, 'print_estimated_vars': True, 'inpa_inj_sched_prev_cumnoise': False, 'schedule_jump_params': {'t_T': 50, 'n_sample': 1, 'jump_length': 10, 'jump_n_sample': 10}}
)
    print('args_sci', args_sci)
    print('args', args.dataroot)
...
out_image = np.array(Image.open('predict/home/out_image.png'))
out_mask = np.array(Image.open('predict/home/out_mask.jpg'))
pg_output_image = Image.open('predict/home/parse_seg_image.png')
...
pg_output = np.sum(pg_output*weights, axis=2)
min_value = np.min(pg_output)
max_value = np.max(pg_output)
scaled_image = (pg_output - min_value) / (max_value - min_value) * 255
scaled_image = scaled_image.astype(np.uint8)
pg_output = Image.fromarray(scaled_image)
print('pg_output uniquw', np.unique(pg_output))


unique_values = np.unique(pg_output)
unique_values.sort()

mapping = {val:i for i,val in enumerate(unique_values)}
print('mapping', mapping)
print('mapping', mapping.get)
mapped_arr = np.vectorize(mapping.get)(pg_output)
parse = torch.from_numpy(np.array(mapped_arr)[None]).long()
parse_13 = torch.FloatTensor(13,1024,768).zero_()
pg_output = parse_13.scatter_(0,parse,1.0)[None]
print('Pg output shape', parse_13.shape)
model_kwargs = {}
...
model_kwargs['y'] = pg_output.clone()
#device = 'cuda'
sci_result = sample_fn(
                ...
                cond_fn=None,
                device=None,
                progress=args_sci.show_progress,
                return_all=True,
                conf=args_sci
                )