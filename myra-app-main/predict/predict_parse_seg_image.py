...
def get_parse_13():
    im_parse_pil_big = Image.open(PARSE_SEG_IMAGE) # 768x1024
    ...
    parse = torch.from_numpy(np.array(im_parse_pil)[:,:0][None]).long()
...
def parse_seg_image_hot_encoder(s_pos: torch.Tensor, p_pos: torch.Tensor):
