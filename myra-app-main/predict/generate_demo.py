...
def draw_part(args, group_id, ten_source, ten_target, ten_source_center, ten_target_center, ten_img):
    ...
    poly = ten_target[group_id].detach().numpy()
...
def test(args, args_sci):
    ...
    kg_network.load_state_dict(torch.load(args.kg_checkpoint_dir,map_location=torch.device('cpu')))
    kg_network.eval()

    pg_network = ParseGenerator(input_nc=19, output_nc=args.output_nc, ngf=64).to(torch.device('cpu'))
    pg_network.load_state_dict(torch.load(args.pg_checkpoint_dir,map_location=torch.device('cpu')))
    pg_network.eval()
